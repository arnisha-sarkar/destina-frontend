...
const LoginPage: React.FC = () => {
  ...
  const onSubmit = async (data: FieldValues) => {
    ...
    try {
      ...
      alert("সার্ভারে কানেক্ট করা যাচ্ছে না। আপনার ব্যাকএন্ড কি রান আছে?");
    } finally {
      setLoading(false);
    }
  };

  // --- ডেমো লগইন হ্যান্ডলার (অটো-সাবমিটসহ) ---
  const handleDemoLogin = (role: "admin" | "user") => {
    ...
    // ১. ফিল্ডগুলো ভ্যালু দিয়ে সেট করা
    setValue("email", target.email);
    setValue("password", target.pass);

    // ২. সরাসরি সাবমিট ফাংশন কল করা
    onSubmit({ email: target.email, password: target.pass });
  };
  ...
  return (
    <div className="relative min-h-screen w-full flex items-center justify-center p-4 overflow-hidden font-sans">
      ...
      <div className="relative z-10 w-full max-w-[1000px] grid grid-cols-1 md:grid-cols-2 bg-white/10 backdrop-blur-2xl rounded-[40px] overflow-hidden shadow-2xl border border-white/20 min-h-[650px]">
        {/* Left Side */}
        <div className="relative hidden md:block group overflow-hidden border-r border-white/10">
          <Image
            src="https://images.unsplash.com/photo-1503220317375-aaad61436b1b?q=80&w=2070"
          ...
          <div className="absolute bottom-12 left-10 text-white">
            ...
            <p className="text-white/70 text-sm max-w-xs mb-6">
              Start your journey today with TravelFlow.
            </p>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input
                  {...register("email", { required: "Email is required" })}
                  className="bg-white/5 border-white/10 text-white h-12 pl-12 rounded-2xl border-none focus-visible:ring-1 focus-visible:ring-white/30 outline-none shadow-none"
                  placeholder="name@example.com"
                />
              </div>
            </div>
            ...
            <Button
              disabled={loading}
              type="submit"
              className="w-full h-12 bg-white text-[#003B73] hover:bg-blue-50 font-bold rounded-2xl shadow-xl mt-2 group transition-all"
            >
              {loading ? "Signing In..." : "Continue"}
              {!loading && (
                <ArrowRight className="ml-2 w-4 h-4 group-hover:translate-x-1 transition-transform" />
            ...
            <div className="pt-4 space-y-3">
              <div className="relative">
                ...
                <div className="relative flex justify-center text-[10px] uppercase">
                  <span className="bg-transparent px-3 text-white/30 font-bold tracking-widest">
                    Demo Access
                  </span>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <Button
                  ...
                  className="h-11 bg-blue-500/10 border border-blue-500/20 text-blue-200 hover:bg-blue-500/20 rounded-xl text-xs gap-2"
                >
                ...
                <Button
                  ...
                  className="h-11 bg-purple-500/10 border border-purple-500/20 text-purple-200 hover:bg-purple-500/20 rounded-xl text-xs gap-2"
                >
